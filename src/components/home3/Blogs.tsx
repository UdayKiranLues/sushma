import Image from "next/image";

const Blogs = () => {
  return (
    <section className="zirox-blog zirox-section">
      <div className="container">
        <h6 data-aos="fade-up" className="text-center">
          From The Blog
        </h6>
        <h2 data-aos="fade-up" data-aos-delay="300" className="text-center">
          Our Latest From The News & Articles
        </h2>
        <div className="row">
          <div data-aos="fade-up" className="col-12 col-md-6 col-lg-4">
            <div className="zirox-blog__post ">
              <div className="zirox-blog__post-thumbnail">
                <a href="#" target="_blank">
                  <Image width={416} height={275} sizes="100vw" style={{width:"100%", height:"auto"}} src="/img/blog/thumbnail-1.png" alt="Post Thumbnail" />
                </a>
              </div>
              <div className="zirox-blog__post-body">
                <div className="zirox-blog__post-body-tags">
                  <div className="zirox-blog__post-body-tags__author">
                    <div className="d-flex align-items-center gap-1">
                      <i className="flaticon-round-account-button-with-user-inside"></i>
                      <p>Admin</p>
                    </div>
                  </div>
                  <div className="zirox-blog__post-body-tags__comment">
                    <div className="d-flex align-items-center gap-1">
                      <i className="flaticon-bubble-chat"></i>
                      <p>2 Comment</p>
                    </div>
                  </div>
                </div>
                <h3 className="zirox-blog__post-body-title">
                  <a href="" title="Post title">
                    The Different Types Of Data Backups
                  </a>
                </h3>
                <p className="zirox-blog__post-body-desc">Duis aute irure dolor in voluptate velit dolore eu fugiat nulla pariatur.</p>
                <a href="#" target="_blank" className="zirox-blog__post-body-read-more">
                  Read More <i className="flaticon-right-arrow-2"></i>
                </a>
              </div>
            </div>
          </div>
          <div data-aos="fade-up" data-aos-delay="300" className="col-12 col-md-6 col-lg-4">
            <div className="zirox-blog__post ">
              <div className="zirox-blog__post-thumbnail">
                <a href="#" target="_blank">
                  <Image width={416} height={275} sizes="100vw" style={{width:"100%", height:"auto"}} src="/img/blog/thumbnail-2.png" alt="Post Thumbnail" />
                </a>
              </div>
              <div className="zirox-blog__post-body">
                <div className="zirox-blog__post-body-tags">
                  <div className="zirox-blog__post-body-tags__author">
                    <div className="d-flex align-items-center gap-1">
                      <i className="flaticon-round-account-button-with-user-inside"></i>
                      <p>Admin</p>
                    </div>
                  </div>
                  <div className="zirox-blog__post-body-tags__comment">
                    <div className="d-flex align-items-center gap-1">
                      <i className="flaticon-bubble-chat"></i>
                      <p>2 Comment</p>
                    </div>
                  </div>
                </div>
                <h3 className="zirox-blog__post-body-title">
                  <a href="" title="Post title">
                    The Different Types Of Data Backups
                  </a>
                </h3>
                <p className="zirox-blog__post-body-desc">Duis aute irure dolor in voluptate velit dolore eu fugiat nulla pariatur.</p>
                <a href="#" target="_blank" className="zirox-blog__post-body-read-more">
                  Read More <i className="flaticon-right-arrow-2"></i>
                </a>
              </div>
            </div>
          </div>
          <div data-aos="fade-up" data-aos-delay="600" className="col-12 col-md-6 col-lg-4">
            <div className="zirox-blog__post ">
              <div className="zirox-blog__post-thumbnail">
                <a href="#" target="_blank">
                  <Image width={416} height={275} sizes="100vw" style={{width:"100%", height:"auto"}} src="/img/blog/thumbnail-3.png" alt="Post Thumbnail" />
                </a>
              </div>
              <div className="zirox-blog__post-body">
                <div className="zirox-blog__post-body-tags">
                  <div className="zirox-blog__post-body-tags__author">
                    <div className="d-flex align-items-center gap-1">
                      <i className="flaticon-round-account-button-with-user-inside"></i>
                      <p>Admin</p>
                    </div>
                  </div>
                  <div className="zirox-blog__post-body-tags__comment">
                    <div className="d-flex align-items-center gap-1">
                      <i className="flaticon-bubble-chat"></i>
                      <p>2 Comment</p>
                    </div>
                  </div>
                </div>
                <h3 className="zirox-blog__post-body-title">
                  <a href="" title="Post title">
                    The Different Types Of Data Backups
                  </a>
                </h3>
                <p className="zirox-blog__post-body-desc">Duis aute irure dolor in voluptate velit dolore eu fugiat nulla pariatur.</p>
                <a href="#" target="_blank" className="zirox-blog__post-body-read-more">
                  Read More <i className="flaticon-right-arrow-2"></i>
                </a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Blogs;
